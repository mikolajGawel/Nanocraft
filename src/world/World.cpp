...
World::World(std::unique_ptr<Terrain> terrain, uint8_t renderDistance, uint8_t decorationRenderDistance) : 
    ...
    detailsRenderDistance(decorationRenderDistance), 
    terrain(std::move(terrain)),
    lastPlayerPosition(glm::ivec2(-648140,-648140))
{
}
World::~World()
{
...
void World::refreshChunks(std::vector<glm::ivec2> chunksToRefresh)
{
...
std::vector<glm::ivec2> World::loadChunks(glm::vec3 position)
{
    ...
    for (int i = -renderDistance; i <= renderDistance; i++)
    {
        for (int j = -renderDistance; j <= renderDistance; j++)
        {
            ...
            if (loadedChunks.find(offsetPos) != loadedChunks.end())
            {
                continue;
            } // if found chunk already on this pos then skip
    ...
    std::vector<glm::ivec2> chunksToRefresh = {};
    for (glm::ivec2 pos : chunkPositions) // updating neighbors in chunks
    {
        checkChunkNeighbors(pos, true);
        chunksToRefresh.push_back(pos);
        for (int i = 0; i < 4; i++)
        { // appends neighbor to refresh queue
            ...
            {
                chunksToRefresh.push_back(neihgborPos);
            }
        }
    }
    for (auto &chunk : loadedChunks)
...
void World::refreshRenderingOrder(glm::ivec2 center){
    renderingOrder.clear();
    for (auto &iter : loadedChunks)
    {
        renderingOrder.push_back(iter.first);
    }
    std::sort(renderingOrder.begin(), renderingOrder.end(), [this,center](const glm::ivec2 &chunkA, const glm::ivec2 &chunkB)
    {
        glm::ivec2 offA = center - chunkA;
        glm::ivec2 offB = center - chunkB;

        float distA = glm::sqrt(pow(offA.x,2) + pow(offA.y,2));
        float distB = glm::sqrt(pow(offB.x,2) + pow(offB.y,2));
        return (distA > distB); 
    });
}
...
void World::update(glm::vec3 position)
{
    ...
    auto chunksToRefresh = loadChunks(position);

    refreshChunks(chunksToRefresh);
    refreshRenderingOrder(_position); 
}
...
void World::render()
{
    for (glm::ivec2 iter : renderingOrder)
    {
        auto optionalChunk = getChunk(iter);
        if (!optionalChunk)
            continue;

        std::shared_ptr<Blocks::Chunk> &chunk = *optionalChunk;
        glEnable(GL_BLEND);
